<?php

/**
 * Contao Open Source CMS
 *
 * @copyright  MEN AT WORK 2014
 * @package    maxmind
 * @license    GNU/LGPL
 * @filesource
 */

/**
 * Legend
 */
$GLOBALS['TL_LANG']['tl_settings']['maxmind_legend']         = 'MaxMind GeoIP settings';

/**
 * Fields
 */

$GLOBALS['TL_LANG']['tl_settings']['maxmind_url']            = array('Donwload URL', 'Please enter the download URL that should be used for downloading the geospatial data for the import.');
$GLOBALS['TL_LANG']['tl_settings']['maxmind_has_license']    = array('MaxMind license key available', 'Click here, if a MaxMind license key is available.');
$GLOBALS['TL_LANG']['tl_settings']['maxmind_license']        = array('License key', 'Please enter the MaxMind license key.');
$GLOBALS['TL_LANG']['tl_settings']['maxmind_lastUpdate']     = array('Last updated', 'This field is automatically filled by the Importer and shows the file of the last successful import.');
$GLOBALS['TL_LANG']['tl_settings']['maxmind_nc']             = array('Email notification', 'Here you can select the setting for sending import notifications (optional).');